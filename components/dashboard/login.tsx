      ...
      <section className="login-panel">
        <div className="login-card">
          ...
          <div className="login-heading">
            ...
            <h2>Welcome back</h2>
            <p>Sign in with an approved team account.</p>
          </div>
          <button
            className={SUPABASE_ENABLED ? "google-button" : "google-disabled"}
            type="button"
            disabled={!SUPABASE_ENABLED}
            onClick={signInWithGoogle}
          >
            <GoogleLogo size={19} weight="bold" /> Continue with Google{" "}
            {!SUPABASE_ENABLED && <span>Setup required</span>}
          </button>
          {!SUPABASE_ENABLED && (
            <>
              ...
              <form onSubmit={submit}>
                ...
                >
                  {loading ? (
                    <>
                      <SpinnerGap className="spin" size={18} /> Signing in
                    </>
                  ) : (
                    <>
