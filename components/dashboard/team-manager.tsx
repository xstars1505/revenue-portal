...
type Invite = { email: string; name: string; role: string; active: boolean; joined: boolean };
const emailPattern = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
const splitEmails = (value: string) => value.split(/[\s,;]+/).map((email) => email.trim().toLowerCase()).filter(Boolean);
...
export function TeamManager() {
  ...
  function commit(value: string) {
    ...
    if (combined.length > 50) { setError("You can invite up to 50 accounts at once"); return; }
    setEmails(combined); setDraft(""); setError("");
  }
  ...
  async function invite(event: FormEvent<HTMLFormElement>) {
    ...
    if (!inviteEmails.length || invalid || inviteEmails.length > 50) { setError(invalid ? `Invalid email: ${invalid}` : "Add between 1 and 50 email addresses"); return; }
    setSaving(true);
  ...
  return <section className="page-section">
    <div className="section-copy team-heading"><div><h2>Approved access</h2><p>Invite Google accounts and assign their portal role.</p></div><Dialog open={open} onOpenChange={(nextOpen) => { setOpen(nextOpen); if (nextOpen) { setError(""); setMessage(""); } }}><DialogTrigger asChild><Button size="lg"><UserPlus />Invite users</Button></DialogTrigger><DialogContent className="sm:max-w-[480px] p-0">
      <DialogHeader className="px-6 pt-6"><DialogTitle>Invite users</DialogTitle><DialogDescription>Add up to 50 Google accounts at once.</DialogDescription></DialogHeader>
      <form className="invite-dialog-form" onSubmit={invite}>
        <div className="invite-dialog-fields">
          <div className="invite-field"><Label htmlFor="invite-emails">Google account emails</Label><div className="email-chip-input" onClick={() => inputRef.current?.focus()}>{emails.map((email) => <Badge variant="secondary" key={email}>{email}<button type="button" aria-label={`Remove ${email}`} onClick={() => setEmails((current) => current.filter((item) => item !== email))}><X size={12} weight="bold" /></button></Badge>)}<Input ref={inputRef} id="invite-emails" value={draft} onChange={(event) => setDraft(event.target.value)} onBlur={() => commit(draft)} onKeyDown={(event) => { if (["Enter", ",", ";", " "].includes(event.key)) { event.preventDefault(); commit(draft); } else if (event.key === "Backspace" && !draft) setEmails((current) => current.slice(0, -1)); }} onPaste={(event) => { const pasted = event.clipboardData.getData("text"); if (splitEmails(pasted).length) { event.preventDefault(); commit(pasted); } }} placeholder={emails.length ? "Add another email" : "name@gmail.com"} inputMode="email" autoComplete="off" /></div><small>Press Enter or paste multiple addresses. Each user must sign in through Google.</small></div>
