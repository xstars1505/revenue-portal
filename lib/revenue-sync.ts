...
async function discoverWorkbooks(accessToken: string, rootId: string) {
  ...
  for (let index = 0; index < folders.length; index++) {
    for (const item of await listDriveFolder(accessToken, folders[index])) {
      ...
      else if (item.mimeType === xlsxMime && /p\s*&\s*l/i.test(item.name))
        files.push(item);
    }
  }
...
async function recordFailure(file: DriveFile, message: string) {
  ...
  const { data } = await admin
    ...
    .single();
  if (data)
    await admin.from("revenue_import_runs").insert({
...
export async function syncRevenueFolder() {
  ...
  const { data: known, error } = await admin
    .from("revenue_source_files")
    .select("drive_file_id,modified_at,checksum,status")
    .in(
      "drive_file_id",
  ...
  const knownById = new Map(
    (known ?? []).map((file) => [file.drive_file_id, file]),
  );
  const result = {
  ...
  for (const file of files) {
    ...
    if (
      ...
      new Date(previous.modified_at).getTime() ===
    ...
    try {
      ...
      if (previous?.status === "imported" && previous.checksum === checksum) {
        await admin
          .from("revenue_source_files")
          .update({ modified_at: file.modifiedTime })
      ...
      const { error: importError } = await admin.rpc(
        "revenue_import_workbook",
        { payload },
      );
      if (importError) throw importError;
      const { error: detailError } = await admin.rpc(
        "revenue_import_workbook_details",
        { payload },
      );
