...
function tokenKey() {
  return createHash("sha256").update(`ledgerly-drive:${required("SUPABASE_SERVICE_ROLE_KEY")}`).digest();
}

export function driveRedirectUri(origin: string) {
  return process.env.GOOGLE_DRIVE_REDIRECT_URI || `${origin}/api/drive/callback`;
}

export function createDriveState(userId: string) {
  const body = Buffer.from(JSON.stringify({ userId, expires: Date.now() + 10 * 60_000 })).toString("base64url");
  const signature = createHmac("sha256", tokenKey()).update(body).digest("base64url");
...
export function verifyDriveState(state: string, userId: string) {
  try {
    ...
    const parsed = JSON.parse(Buffer.from(body, "base64url").toString()) as { userId?: string; expires?: number };
...
function decrypt(value: string) {
  ...
  if (!iv || !tag || !encrypted) throw new Error("Stored Drive credential is invalid");
  const decipher = createDecipheriv("aes-256-gcm", tokenKey(), iv);
  decipher.setAuthTag(tag);
  return Buffer.concat([decipher.update(encrypted), decipher.final()]).toString("utf8");
}

export async function saveDriveCredential(refreshToken: string, accessToken: string) {
  const userResponse = await fetch("https://openidconnect.googleapis.com/v1/userinfo", { headers: { authorization: `Bearer ${accessToken}` } });
  const googleUser = await userResponse.json() as { email?: string };
  if (!userResponse.ok || !googleUser.email) throw new Error("Could not identify the connected Google account");
  const { error } = await createAdminClient().from("revenue_drive_credentials").upsert({
    id: true,
...
export async function getDriveAccessToken() {
  const { data, error } = await createAdminClient().from("revenue_drive_credentials").select("refresh_token_encrypted").eq("id", true).maybeSingle();
  if (error) throw error;
...
export async function listDriveFolder(accessToken: string, folderId: string) {
  ...
  do {
    ...
    const body = await response.json() as { files?: DriveFile[]; nextPageToken?: string; error?: { message?: string } };
    if (!response.ok) throw new Error(body.error?.message || "Could not list Google Drive folder");
...
export async function downloadDriveFile(accessToken: string, fileId: string) {
  const response = await fetch(`https://www.googleapis.com/drive/v3/files/${encodeURIComponent(fileId)}?alt=media&supportsAllDrives=true`, { headers: { authorization: `Bearer ${accessToken}` } });
  if (!response.ok) {
    const body = await response.json().catch(() => null) as { error?: { message?: string; errors?: { reason?: string }[] } } | null;
    if (body?.error?.errors?.some((error) => error.reason === "cannotDownloadFile")) throw new Error("The Drive owner has disabled downloads for this file. Ask the owner to allow downloads or replace it with a downloadable copy.");
