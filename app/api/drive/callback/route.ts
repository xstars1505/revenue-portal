...
import { exchangeDriveCode, listDriveFolder, saveDriveCredential, verifyDriveState } from "@/lib/google-drive";
...
export async function GET(request: NextRequest) {
  ...
  if (!authorized || !code || !state || !verifyDriveState(state, authorized.user.id)) return NextResponse.redirect(`${origin}/workbooks?drive=error`);
  try {
    ...
    if (!process.env.GOOGLE_DRIVE_FOLDER_ID) throw new Error("GOOGLE_DRIVE_FOLDER_ID is missing");
    await listDriveFolder(tokens.access_token!, process.env.GOOGLE_DRIVE_FOLDER_ID);
