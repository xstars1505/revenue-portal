...
export async function GET() {
  ...
  return NextResponse.json({ users: (invites ?? []).map((invite) => ({ ...invite, name: names.get(invite.email.toLowerCase())?.display_name ?? invite.email.split("@")[0], joined: names.get(invite.email.toLowerCase())?.active === true })) });
}
...
export async function POST(request: Request) {
  ...
  const sent = deliveries.filter(({ error: inviteError }) => !inviteError).map(({ email }) => email);
  if (sent.length) {
    const { error } = await admin.rpc("revenue_upsert_invites", { invite_emails: sent, invite_role: role });
    if (error) return NextResponse.json({ error: `Invitation email sent, but access could not be prepared: ${error.message}` }, { status: 500 });
  }
  if (failed.length) {
    const rateLimited = failed.some(({ error: inviteError }) => inviteError!.status === 429 || inviteError!.message.toLowerCase().includes("rate limit"));
    ...
    return NextResponse.json({ error, invited: sent.length, sent, failed: failed.map(({ email }) => email) }, { status: rateLimited ? 429 : 502 });
  }
...
export async function DELETE(request: Request) {
  ...
  if (email.length > 254 || !emailPattern.test(email)) return NextResponse.json({ error: "Enter a valid email address" }, { status: 400 });
  const { error } = await createAdminClient().rpc("revenue_remove_user", { target_email: email });
