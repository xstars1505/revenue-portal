...
export default function WorkbooksPage() {
  ...
  return (
    <section className="page-section">
      <div className="section-copy workbook-heading">
        <div>
          <h2>Monthly workbooks</h2>
          <p>Source files used for the current reporting history.</p>
        </div>
        {status?.connected && canSync && (
          <Button onClick={refresh} disabled={syncing}>
            {syncing ? <SpinnerGap className="spin" /> : <ArrowClockwise />}{" "}
      ...
      <div className="folder-card">
        ...
        <div>
          <strong>Finance / Monthly P&amp;L</strong>
          <span>
            {status?.connected
      ...
      <div className="panel workbook-list">
        {[...months].reverse().map((month, index) => (
          <div key={`${month.month}-${month.source}`}>
            <span className="file-tile">
              <Files size={20} weight="fill" />
            </span>
            <div>
              <strong>{month.source}</strong>
              <span>{month.month} · Central branch</span>
            </div>
            <b>{money(month.revenue, true)}</b>
            <span className={index === 0 ? "status-badge" : "muted-badge"}>
